...
export async function POST(req: Request) {
  ...
  // Security practice: chahe user na mile, hamesha same success message do —
  // taake koi ye pata na laga sake ke konsi emails registered hain.
  if (!user) {
    return NextResponse.json({ message: "Agar account exist karta hai, email bhej di gayi hai." });
  }

  const token = crypto.randomBytes(32).toString("hex");
  const expiresAt = new Date(Date.now() + 60 * 60 * 1000); // 1 ghanta
